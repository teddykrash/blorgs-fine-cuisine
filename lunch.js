let pageBody = document.querySelector("body");
function buildMenu(foodData){
    //regular lunch items
    let regularLunchList = document.querySelector("#regularLunchPrice");
    let regularLunchItems = foodData.lunch;
    for(let i=0;i<regularLunchItems.length;i++){
        let foodItem = document.createElement("li");

        let topRow = document.createElement("div");
        topRow.classList.add("menu-item-top");

        let itemName = document.createElement("span");
        itemName.classList.add("item-name");
        itemName.innerText = regularLunchItems[i].food_name;

        let itemPrice = document.createElement("span");
        itemPrice.classList.add("item-price");
        if(regularLunchItems[i].is_combo == "1"){
            itemPrice.innerText = `$${regularLunchItems[i].price} (Combo: $${regularLunchItems[i].combo_price})`;
        } else {
            itemPrice.innerText = `$${regularLunchItems[i].price}`;
        }

        topRow.appendChild(itemName);
        topRow.appendChild(itemPrice);
        foodItem.appendChild(topRow);

        let itemDesc = document.createElement("p");
        itemDesc.classList.add("item-desc");
        itemDesc.innerText = `${regularLunchItems[i].desc} — ${regularLunchItems[i].calories} cal`;
        foodItem.appendChild(itemDesc);

        regularLunchList.appendChild(foodItem);
    }

    //value lunch items
    let valueLunchList = document.querySelector("#valueLunchPrice");
    let valueLunchItems = foodData.valueLunch;
    for(let i=0;i<valueLunchItems.length;i++){
        let foodItem = document.createElement("li");

        let topRow = document.createElement("div");
        topRow.classList.add("menu-item-top");

        let itemName = document.createElement("span");
        itemName.classList.add("item-name");
        itemName.innerText = valueLunchItems[i].food_name;

        let itemPrice = document.createElement("span");
        itemPrice.classList.add("item-price");
        if(valueLunchItems[i].is_combo == "1"){
            itemPrice.innerText = `$${valueLunchItems[i].price} (Combo: $${valueLunchItems[i].combo_price})`;
        } else {
            itemPrice.innerText = `$${valueLunchItems[i].price}`;
        }

        topRow.appendChild(itemName);
        topRow.appendChild(itemPrice);
        foodItem.appendChild(topRow);

        let itemDesc = document.createElement("p");
        itemDesc.classList.add("item-desc");
        itemDesc.innerText = `${valueLunchItems[i].desc} — ${valueLunchItems[i].calories} cal`;
        foodItem.appendChild(itemDesc);

        valueLunchList.appendChild(foodItem);
    }

    // Dessert items
    let dessertList = document.querySelector("#desserts");
    let dessertItems = foodData.desserts;
    for(let i=0;i<dessertItems.length;i++){
        let foodItem = document.createElement("li");

        let topRow = document.createElement("div");
        topRow.classList.add("menu-item-top");

        let itemName = document.createElement("span");
        itemName.classList.add("item-name");
        itemName.innerText = dessertItems[i].food_name;

        let itemPrice = document.createElement("span");
        itemPrice.classList.add("item-price");
        if(dessertItems[i].is_combo == "1"){
            itemPrice.innerText = `$${dessertItems[i].price} (Combo: $${dessertItems[i].combo_price})`;
        } else {
            itemPrice.innerText = `$${dessertItems[i].price}`;
        }

        topRow.appendChild(itemName);
        topRow.appendChild(itemPrice);
        foodItem.appendChild(topRow);

        let itemDesc = document.createElement("p");
        itemDesc.classList.add("item-desc");
        itemDesc.innerText = `${dessertItems[i].desc} — ${dessertItems[i].calories} cal`;
        foodItem.appendChild(itemDesc);

        dessertList.appendChild(foodItem);
    }

    // Side items
    let sidesList = document.querySelector("#sides");
    let sideItems = foodData.side;
    for(let i=0;i<sideItems.length;i++){
        let foodItem = document.createElement("li");

        let topRow = document.createElement("div");
        topRow.classList.add("menu-item-top");

        let itemName = document.createElement("span");
        itemName.classList.add("item-name");
        itemName.innerText = sideItems[i].food_name;

        let itemPrice = document.createElement("span");
        itemPrice.classList.add("item-price");
        if(sideItems[i].is_combo == "1"){
            itemPrice.innerText = `$${sideItems[i].price} (Combo: $${sideItems[i].combo_price})`;
        } else {
            itemPrice.innerText = `$${sideItems[i].price}`;
        }

        topRow.appendChild(itemName);
        topRow.appendChild(itemPrice);
        foodItem.appendChild(topRow);

        let itemDesc = document.createElement("p");
        itemDesc.classList.add("item-desc");
        itemDesc.innerText = `${sideItems[i].desc} — ${sideItems[i].calories} cal`;
        foodItem.appendChild(itemDesc);

        sidesList.appendChild(foodItem);
    }
}
function buildTitle(){
    let titleHolder = document.createElement("header");

    let logo = document.createElement("img");
    logo.src = "images/logo.png";
    logo.alt = "Blorgs Fine Cuisine Logo";
    logo.classList.add("menu-logo");
    titleHolder.appendChild(logo);

    let titleEl = document.createElement("h1");
    titleEl.innerText = "WELCOME TO BLORGS FINE CUISINE";
    titleHolder.appendChild(titleEl);

    pageBody.appendChild(titleHolder);
}
function buildFoodHolders(){
    let regularLunchTitle = document.createElement("h2");
    regularLunchTitle.innerText = "Regular Lunch Menu";
    pageBody.appendChild(regularLunchTitle);
    let regularLunch = document.createElement("ul");
    regularLunch.id ="regularLunchPrice";
    pageBody.appendChild(regularLunch);
    
    let valueLunchTitle = document.createElement("h2");
    valueLunchTitle.innerText = "Affordable Lunches";
    pageBody.appendChild(valueLunchTitle);
    let valueLunchPrice = document.createElement("ul");
    valueLunchPrice.id ="valueLunchPrice";
    pageBody.appendChild(valueLunchPrice);

    let dessertTitle = document.createElement("h2");
    dessertTitle.innerText = "Desserts";
    pageBody.appendChild(dessertTitle);
    let desserts = document.createElement("ul");
    desserts.id = "desserts";
    pageBody.appendChild(desserts);

    let sidesTitle = document.createElement("h2");
    sidesTitle.innerText = "Side Dishes";
    pageBody.appendChild(sidesTitle);
    let sides = document.createElement("ul");
    sides.id = "sides";
    pageBody.appendChild(sides);
    getData();
}
function clearFoodList(){
    document.querySelector("#foodList").innerHTML="";
}

async function getData() {
    const url = "./services/full_lunch_api.php";
    try {
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(`Response status: ${response.status}`);
        }

        const result = await response.json();
        console.log(result);
        console.log("the lunch items");
        console.log(result.lunch);
        console.log("the value lunch items");
        console.log(result.valueLunch);
        buildMenu(result);
    } catch (error) {
        console.error(error.message);
    }
}

function init(){
    buildTitle();
    buildFoodHolders();
}
init();